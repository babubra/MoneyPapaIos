...
| **C1+C2-fixups** | 🔄 In progress | 2026-05-14 (Opus 4.7) | M6 + #4/#5/#7/#16/**#17** + **M14** + **M13** + **#3** + **#15** + **#10** закрыты. **#1 (prompt-cache)** переоткрыт: aitunnel не делает prefix-cache → отложено. **#10 (audio duration validation)** закрыт: `mutagen` парсит header → 413 если duration > `AI_MAX_AUDIO_SECONDS=30`, 422 если файл не парсится. 7 unit-тестов + curl-смоук 3 сценария. Дальше: **#2** (Device cap) → хвост 🟢. См. [`audit/C1_C2_ai_layer.md`](audit/C1_C2_ai_layer.md) |
| C3 | ⬜ TODO | — | — |
...
- 2026-05-14 — закрыт **#10 (audio duration validation)** в C1+C2-fixups (Opus 4.7, продолжение сессии `c-generic-wilkes`). Без server-check 5-минутный AAC 16 kHz моно укладывался в `AI_MAX_AUDIO_SIZE_MB=5` cap и пожирал ~7 500 input-токенов multimodal на запрос; с новым Premium-cap=50/сутки атакующий через модифицированный клиент мог слить 50 × 7 500 = 375 000 токенов в сутки. **Backend:** добавлен `mutagen==1.47.0` (pure-Python audio header parser, ~50 KB — без ffmpeg-зависимости в Docker); новый helper `_get_audio_duration_seconds(audio_data: bytes) -> float` в `ai.py` (mutagen.File header parse, raises 422 если `None`/`info=None`/`length<=0`/MutagenError); вставлен check в `parse_audio` сразу после `audio.read()` size-check — если duration > `AI_MAX_AUDIO_SECONDS=30` → 413 с заголовками `X-Audio-Duration` + `X-Audio-Duration-Limit`. **iOS не трогали** — AIService уже декодирует `detail` из 4xx ответов и показывает юзеру (`AIServiceError.serverError → error.localizedDescription`); отдельная локализация для 413/422 не требуется (дефолтный fallback читает `detail`). **Тесты:** новый `backend/tests/test_audio_duration.py` (7 кейсов: валидное parse возвращает float, граничные duration=0/<0 → 422, mutagen.File→None → 422, info без length → 422, MutagenError → 422, info=None → 422). Все мокаются через `patch("app.api.v1.ai.mutagen.File")` — независимо от наличия audio-fixtures в репо. Mock-сьют 64 → 71 passed. **Образ пересобран** (`docker compose up -d --build`) — mutagen в venv и в Docker. **Curl-смоук 3 сценария** с ffmpeg-генерированным AAC 16 kHz моно: short 5s (34 KB) → 200, long 45s (290 KB) → 413 с `X-Audio-Duration: 45.06`, malformed bytes → 422 «Неизвестный формат аудио».
- 2026-05-14 — закрыт **#15 (Premium daily-cap)** в C1+C2-fixups (Opus 4.7, продолжение сессии `c-generic-wilkes`). Финансовый safety-net против угнанного Premium-JWT и retry-loop'ов на клиенте. **Backend:** 3 новых поля в `User` (`ai_requests_today`, `ai_audio_requests_today`, `ai_requests_reset_date` String YYYY-MM-DD UTC); 2 новых конфига `AI_PREMIUM_DAILY_CAP_TEXT=200`/`AUDIO=50`; helper'ы `_utc_today_str()` + `_reset_premium_counters_if_needed()` (lazy reset при первом запросе нового UTC-дня); расширены `_check_trial(user, db, is_audio=False)` и `_consume_trial(user, db, is_audio=False)` — Premium-юзеры получают 429 с заголовками `X-AI-Daily-Used/Limit/Kind` при исчерпании; free-flow по-прежнему 402, без изменений. **Заодно закрыт #17** — удалены мёртвые `AI_RATE_LIMIT_DAILY` / `AI_RATE_LIMIT_AUDIO_HOURLY` из `config.py` и `.env.example`. **iOS не трогали** — он уже различает 402 (`paymentRequired`) и 429 (`rateLimitExceeded` → `error.rateLimit`). **Тесты:** новый `backend/tests/test_premium_daily_cap.py` (18 кейсов: free-flow regression, Premium text/audio под/на cap, кросс-проверка независимости text/audio cap, auto-reset на новой UTC-дате, истёкшая подписка → fallback на 402, бессрочный Premium с `expires_at=NULL`); mock-сьют 64 passed (было 51). **БД пересоздана** через `docker compose down -v && up -d --build` (dev-flow без Alembic). **Curl-смоук 4 сценария:** (1) free → `ai_trial_used`+1, Premium-поля 0; (2) Premium → `ai_requests_today`+1, `reset_date` проставлен; (3) Premium на cap=200 → 429 с правильным телом и заголовками; (4) Premium с `reset_date=1999-01-01` → auto-reset, request проходит. **Audio-flow в curl не покрыли** (multipart-сложность) — покрыто unit-тестами. **Не делали:** Alembic-миграцию (D1, потом) — на prod добавление 3 полей потребует ручной миграции или D1.
