...
| **C1+C2-fixups** | 🔄 In progress | 2026-05-14 (Opus 4.7) | M6 + #4/#5/#7/#16/**#17** + **M14** + **M13** + **#3** + **#15** закрыты. **#1 (prompt-cache)** переоткрыт: aitunnel не делает prefix-cache → отложено. **#15 (Premium daily-cap)** закрыт: 3 поля в `User`, `AI_PREMIUM_DAILY_CAP_TEXT=200`/`AUDIO=50`, lazy UTC-reset, 429 для Premium, free-flow не задет. 18 unit-тестов + curl-смоук 4 сценария. Дальше: **#10** (audio duration) → **#2** (Device cap) → хвост 🟢. См. [`audit/C1_C2_ai_layer.md`](audit/C1_C2_ai_layer.md) |
| C3 | ⬜ TODO | — | — |
