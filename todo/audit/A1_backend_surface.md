...
> ⚠️ **Update 2026-05-04 — изменилась auth-модель.** Принято решение перейти на схему **C** (обязательная авторизация + AI trial + платная sync). См. план миграции: [`../auth_model_C_migration.md`](../auth_model_C_migration.md).
>
> Часть критов из этого отчёта закрывается архитектурно после миграции:
> - 🔴 «фабрика device_id» (`/auth/device` без rate-limit) → ✅ эндпоинт удаляется
> - 🔴 DEV_MODE auto-login → 🟡 частично (DEV_MODE остаётся, но без анонимного device-режима)
>
> Остальные крит/medium валидны при любой архитектуре и **должны быть зафикшены отдельно**, до или параллельно с миграцией C.
