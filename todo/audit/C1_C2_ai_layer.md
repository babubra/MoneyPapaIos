...
|----|----------|--------|------|
| 1  | 🔴 (re-open 2026-05-12) | Нет prompt caching — попытка стабилизации провалилась, aitunnel не делает prefix-cache. Требуется смена провайдера (Gemini direct / Anthropic Claude). См. подробный пост-мортем в «Статус закрытия» от 2026-05-12. | `ai.py:222-233`, `system_prompt.py` |
| 2  | 🔴 | Trial-bypass + Device.ai_requests_today мёртвый | `ai.py:82-127`, `models.py:99-107` |
| 3  | ✅ 🔴 | retry + идемпотентность (закрыт 2026-05-13): backend `IdempotencyStore` (in-memory TTL=60s) + iOS `executeWithRetry` с backoff `[0,1s,3s]` на URLError/502/503/504 + Idempotency-Key header. Verified: 2 запроса с одним key → AI вызван 1 раз, trial не списан повторно. | `ai.py:75-118`, `core/idempotency.py`, `AIService.swift:117-118,141,194,224-275` |
...
| 13 | ✅ 🟡 | SYSTEM_PROMPT redundancy (закрыт 2026-05-12, M13: -14% avg prompt_tokens на golden, по самому SYSTEM_PROMPT -12% символов / -10% байт) | `system_prompt.py` |
| 14 | ✅ 🟡 | Нет per-locale evals + нет тестов (закрыт 2026-05-11: M14 — pytest-инфра, mock-слои + golden-сьют на реальный aitunnel, фокус на долгах) | `backend/tests/` |
...
- 2026-05-11: ✅ **#14 (M14)** — eval-инфраструктура. Создано `backend/tests/`: pytest scaffolding, 31 mock-тест (unit + retry-логика + 6 regression-эвалов) + 15 golden-кейсов на реальный aitunnel (`@pytest.mark.golden`, ~$0.03 за прогон). Главный фокус — **долги**: 8 кейсов покрывают `debt_take`/`debt_give`/`debt_payment` + fuzzy-matching counterpart (Серёжа↔Сергей, Mike↔Michael) + payment_flow + ambiguous-counterparts. **14/15 golden green с первого прогона**; xfail зафиксированы как red-тесты для **#19** (sanitize режет URL), **#21** (locale fallback) и одного нового C3-finding (см. ниже). Итог: 57 passed, 4 xfailed за ~22 секунды.
- **Корректировка к #1 (prompt caching):** golden-прогон показал, что у aitunnel/gemini-2.5-flash-lite `prompt_tokens_details.cached_tokens` **на самом деле работает**, но не на каждом вызове. В `ru_debt_payment_ambiguous` и `ru_off_topic` (последние два запроса подряд) `cached_tokens=2980-2981`, остальные `cached=0`. Это сильно меняет дизайн фикса #1: задача не «включить caching», а «довести его до стабильного попадания» (вероятно, через `prompt_cache_key` или повторяемость SYSTEM_PROMPT). Baseline для дальнейших экспериментов — `_log_ai_usage` теперь в формате `prompt=N completion=N total=N cached=N`.
- **Новый finding для C3** (зарегистрирован 2026-05-11 при прогоне M14): SYSTEM_PROMPT раздел 2 говорит «If amount is missing: status=incomplete, missing=[amount]», но gemini-2.5-flash-lite на «купил хлеб» (без суммы) возвращает `status="ok", amount=null`. Покрыто xfail-кейсом `tests/golden/cases.py::ru_incomplete_amount`. Будет исправлено в C3-сессии усилением формулировки раздела 2.
- **Дополнительный наблюдение для C3:** на `ru_lemmatization` («купил вкусную колбасу за 300» + cat `Продукты`) модель вернула `category_name="Продукты", category_is_new=false, category_id=null`. То есть промпт раздел 4 «Match existing category by name/meaning» не явно требует ставить `category_id` когда найдено совпадение. Кейс смягчён через `assert_in {category_id: ["c1", null]}`, чтобы не мешать прогону, но это явный сигнал для C3.
- 2026-05-12: ✅ **#13 (M13)** — сокращение SYSTEM_PROMPT. 4 этапа: (A) убраны кричащие маркеры `(CRITICAL)`/`(IMPORTANT)`/`(REQUIRED in response)` в заголовках 4b/6/7/8/9; (B) раздел 10 — три почти одинаковых JSON-объекта сжаты в один base-shape + word-level type-specific notes; (C) раздел 8 — debt-примеры с 12 до 8 (убраны лексические дубликаты «погасил/рассчитался», правила `## CRITICAL вернул/отдал`, `## Key rules` сохранены); (D) раздел 4b — 5 строк → 2, safety-net остаётся в `_normalize_null_strings`. **Метрики:** `system_prompt.py` 13705→12350 байт (-1355, -9.9%); `SYSTEM_PROMPT` text 9657→8488 символов (-1169, -12.1%); средний `prompt_tokens` на golden 3143→2710 (-433, -13.8%). Mock-слой 43 passed (без изменений), golden 14 passed + 1 xfailed (без изменений). Каждый этап — отдельный коммит, откатываемый независимо.
- 2026-05-12: 🟥 **#1 prompt caching — попытка стабилизации провалилась, корневая причина выяснена**. Сессия `flickering-prancing-prism` (Opus 4.7). Расширил `backend/check_cache.py` режимами `--repeat N`, `--user-key STR`, `--vary` для контролируемых экспериментов. Прогоны (~$0.15 на 41 запросов):
  - **Identical prompts, no hints, cold start:** 6/10 hits (60%) — первые 3 запроса miss, дальше cache прогрелся.
  - **Identical prompts, with `user=u0` + `extra_body.prompt_cache_key=u0`:** 8/10 hits (80%) — но cache был уже прогрет.
  - **Identical prompts, no hints, immediately after:** 9/10 hits (90%) — тоже на прогретом cache.
  - **`--vary` (разные user_texts) + `user=u0` + `prompt_cache_key=u0`:** **0/10 hits** — главный замер.
  - **Golden suite (15 разных prompts) с правкой hints в ai.py:** **0/15 hits** — подтверждение.
  - **Корневая причина:** aitunnel **не делает префиксное кэширование** SYSTEM_PROMPT. Имплицит-кэш Gemini у них либо отключён, либо работает только на полный exact-match по всей последовательности messages. M14-наблюдение «2/15 hits с cached=2980» теперь интерпретируется как случайное совпадение полного messages-payload (одна и та же тестовая параметризация может давать идентичные user_prompt'ы в краевых случаях). Steady-state «80% hits» у меня в check_cache.py был **артефактом идентичных запросов**, не репрезентативной картиной prod-трафика.
  - **Что сделано:** правка `user=`/`extra_body` в `_call_ai_text` и `_call_ai_audio` **откатана** — на реальном трафике (разные user_texts) она не давала эффекта, оставлять мёртвый код хуже отсутствующего. Mock-тест `test_passes_routing_hints` тоже снят. Сохранено: расширенный `backend/check_cache.py` (диагностический инструмент, режим `--vary` критичен для будущих экспериментов с любым провайдером) — это и есть основной артефакт сессии.
  - **Статус #1:** **остаётся открытым** с переформулированной рекомендацией. **Не «стабилизация»**, а **«получить prefix-caching как явный механизм»** — что через aitunnel openai-compat невозможно без поддержки с их стороны. Альтернативы: (а) прямая интеграция с Gemini API (`cached_content` resource — нужна Google Cloud-настройка), (б) переход на Anthropic Claude API (`cache_control` breakpoints на system message — самый чистый API для prefix-cache, требует Anthropic-аккаунт), (в) подождать, пока aitunnel добавит поддержку. **Магнитуда проблемы подтверждена через aitunnel-биллинг:** miss = 0.07 руб/запрос, hit = 0.04 руб/запрос → ~1.75× разница. При 1000 запросов/день и 0% hit rate (текущая реальность) — это полная цена за весь SYSTEM_PROMPT каждый раз.
  - **Что не делалось намеренно:** не уходили с aitunnel — это отдельная задача (РФ-окружение, выбор альтернативного прокси, прямая интеграция). Не пытались форсить exact-match через хитрости в prompt — это не масштабируется (каждый юзер уникален). Не открывали тикет в aitunnel — это пользовательский шаг, не аудитный.
  - **Артефакт сессии для будущих фиксов:** `python check_cache.py --repeat 10 --vary` — однострочник, который объективно покажет, есть ли prefix-caching у любого AI-провайдера, на который мы будем смотреть в будущем. Хранить как regression-инструмент.
- 2026-05-13: ✅ **#3 (retry + идемпотентность)** — закрыт. Сессия `flickering-prancing-prism` (продолжение, Opus 4.7). Двусторонний фикс:
  - **Backend:** новый модуль [`app/core/idempotency.py`](../../backend/app/core/idempotency.py) с `IdempotencyStore` (in-memory TTL=60s, asyncio.Lock-защищённый); состояния `NEW`/`IN_FLIGHT`/`CACHED`. Стор инстансится в `lifespan` (`main.py:38`), достаётся через FastAPI-dependency `get_idempotency_store`. Endpoints `/parse` и `/parse-audio` принимают header `Idempotency-Key` (parsing через `_resolve_idempotency_key` с max-len 128 и user_id-scoped composite key для изоляции). Поведение: cached → возврат закэшированного ответа без AI-вызова и без двойного `_consume_trial`; in_flight → 409 Conflict; AI-fail → `release_failure` → юзер может ретраить. Без header — back-compat, старые клиенты работают как раньше.
  - **iOS-клиент:** [`AIService.swift`](../../Monpapa/Monpapa/Services/AIService.swift) — генерация `idempotencyKey = UUID().uuidString` **один раз** на user-action в `parseText`/`parseAudio`, выставление header `Idempotency-Key`. Новый helper `executeWithRetry` с exponential backoff `[0, 1s, 3s]` (3 попытки), ретраит на URLError-сетевых (`.timedOut`, `.networkConnectionLost`, `.notConnectedToInternet`, `.cannotConnectToHost`, `.cannotFindHost`, `.dnsLookupFailed`) и HTTP 502/503/504. НЕ ретраит на 4xx (401/402/409/422/429) и нефлапающих 5xx — это валидные финальные отказы.
  - **Тесты:** 8 unit-тестов на стор (`tests/test_idempotency.py`) — NEW/IN_FLIGHT/CACHED/TTL-expiry/release/concurrency. Mock-сьют 43→51 passed (8 новых), xfailed=3 без изменений. Golden не перезапускался — фикс на уровне endpoint-orchestration, не затрагивает `_call_ai_text`/SYSTEM_PROMPT.
  - **Verified curl-сценариями:** (а) 2 запроса с одним `Idempotency-Key` → 1-й вызывает AI (`AI usage | mode=text prompt=2699 cached=0`), 2-й `PARSE TEXT idempotent | state=cached` **без AI usage-лога** (значит AI НЕ вызван), `trial=1/50` после обоих (не списан дважды). (б) 3-й запрос без header → back-compat: AI вызван, trial 1→2.
  - **Магнитуда:** на flaky-network юзер раньше мог родить N billable AI-calls (по одному на retap), теперь — 1 на user-action независимо от ретраев в окне 60s. Особенно важно для Premium-юзеров (#15) — без daily-cap каждый retap = реальные деньги OpenAI.
  - **Что НЕ делалось:** не добавлен Redis-стор (in-memory достаточно для single-worker uvicorn, для multi-worker нужен будет рефактор); не добавлен handler для 409 в iOS-клиенте (попадает в дефолтный `.serverError(409)` → юзер увидит generic-ошибку, при следующем тапе новый key и всё работает); не добавлены automated iOS-тесты (нет тестового стенда в Monpapa.xcodeproj, верификация ручная через Xcode Network Link Conditioner).
- Открыто: #1 (переоткрыт), #2, #8-#12, #15, #17-#21. Дальше по приоритету: **#15** (Premium daily-cap — закрывает другой вектор утечки денег, не требует смены провайдера) → **#10** (audio duration validation) → **#2** (Device.ai_requests_today для trial-bypass) → хвост 🟢. **#1 переоткрыт** до момента, когда будет решение по провайдеру.
