...
## 🛡️ Безопасность Backend (Antispam & Rate Limiting)
Так как API публично доступен, необходимо внедрить многоуровневую защиту от ботов, перебора и спам-атак (особенно для эндпоинтов авторизации и AI-генерации).

### 1. Троттлинг на уровне IP-адресов
- **Цель**: Предотвратить массовый автоматизированный перебор с одного IP.
- **Реализация**: Использование библиотек `fastapi-limiter` + `Redis` (или in-memory кэш).
- **Лимиты**:
  - `/api/v1/auth/request-link` — макс. 5-10 запросов в минуту с одного IP.
  - Общесистемные CRUD-эндпоинты — макс. 100-200 запросов в минуту с одного IP.

### 2. Защита почтовых эндпоинтов (Email Throttling)
- **Цель**: Не допустить использования бэкенда для рассылки спама на чужие адреса (сохранение репутации и лимитов почтового сервиса).
- **Реализация**: Сохранение метки времени `last_magic_link_sent_at` в БД или кэше для каждого запрашиваемого `email`.
- **Лимиты**:
  - Жёсткий кулдаун 60 секунд между запросами кода на **один и тот же email** (сервер должен отдавать `HTTP 429 Too Many Requests`).
  - Абсолютный лимит писем: не более 5-7 писем на один email в течение 1 часа.

### 3. Фильтрация и защита AI-точек (Cost Protection)
- **Цель**: Защита от истощения баланса API-провайдера (AiTunnel / Gemini API).
- **Реализация**: Подсчёт потребления токенов и запросов в привязке к `deviceId` или авторизованному `user_id`. Строгое соблюдение лимитов (50 текст / 5 аудио), немедленный отказ при их превышении с возвратом `HTTP 429`.

### 4. На будущее (Advanced Security)
- **DeviceCheck / App Attest**: Проверка на уровне серверов Apple, доказывающая, что запрос исходит с подлинного iPhone и легитимного приложения (полностью исключает спам обычными скриптами).
- **Cloudflare Turnstile**: Для Web-версии — невидимая проверка на ботов при попытке входа по почте.

---

## 🕵️‍♂️ Приватность и Политика Apple (TODO)
Необходимо проработать оптимальный вариант хранения пользовательских данных (особенно `email`) в свете требований App Store (Guideline 5.1.1 Data Collection and Storage).

### Варианты для окончательного решения:
- [ ] **Стратегия 1 (Классическая)**: Хранение `email` в открытом виде для базовой техподдержки и возможности рассылок. Требует наличия публичной Privacy Policy с указанием целей сбора и обязательной кнопки «Удалить аккаунт».
- [ ] **Стратегия 2 (Zero-Knowledge / Киберпанк)**: Хранение **только SHA-256 хеша** от почты с солью. Письма с кодом отправляются "на лету" из оперативной памяти бэкенда, не оседая на диске. Это делает финансовые данные доказуемо анонимными (можно использовать для маркетинга "Мы не знаем кто вы"), но исключает любые новостные рассылки пользователям.

**Ближайшие шаги (Privacy Action Items):**
- Окончательно утвердить один из вариантов хранения (Хеш vs Текст) до релиза бэкенда в Production.
- Сгенерировать и разместить Privacy Policy.
- Убедиться, что функционал полного удаления профиля и транзакций с серверов Apple & iOS работает безупречно.
