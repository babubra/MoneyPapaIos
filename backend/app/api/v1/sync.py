...
async def sync_batch(
    ...
    Доступно ВСЕМ авторизованным юзерам, без подписочного gate. Sync —
    это «гигиена», бэкап локальных данных в облако. Premium продаёт AI
    (см. trial-gate в ai.py), а не синхронизацию: терять финансовые
    данные при переустановке приложения — плохой UX, и вынуждать за это
    платить — плохая бизнес-этика.

    Решение зафиксировано в `todo/auth_model_C_migration.md` (вариант A).
